...
def main(page: ft.Page):
    ...
    def create_header(title, subtitle_control=None, leading_action=None, trailing_action=None):
        """
        Crea una barra superior. 
        Nota: subtitle_control debe ser un ft.Control (como ft.Text) si quieres actualizarlo,
        o un string simple.
        """
        subtitle_widget = subtitle_control
        if isinstance(subtitle_control, str):
            subtitle_widget = ft.Text(subtitle_control, size=12, color="white70")
        elif subtitle_control is None:
    ...
    class AdminService:
        ...
        @staticmethod
        def delete_ciclo(cid): db_delete_ciclo(cid)
        ...
        return ft.View("/", [
            ft.Container(
                content=ft.Column([
                    ...
                    ft.ElevatedButton("INICIAR SESIÓN", on_click=login_click, width=300, height=45, bgcolor=PRIMARY_COLOR, color="white", style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10), elevation=5)),
    ...
    def view_dashboard():
        # CONTROL DE TEXTO DINÁMICO para el Ciclo
        # Esto asegura que si el ciclo cambia en la BD, este texto se actualice al llamar load_dashboard_data
        text_ciclo_status = ft.Text("Cargando...", weight=ft.FontWeight.BOLD, color="white70", size=12)
        ...
        # Referencia mutable para el ID del ciclo
        current_cycle_ref = {"id": None}
        ...
        def load_dashboard_data():
            ...
            # Consultar siempre a la BD para tener el dato fresco
            ciclo = run_query_one("SELECT * FROM Ciclos WHERE activo = 1")
            
            if ciclo:
                current_cycle_ref["id"] = ciclo['id']
                text_ciclo_status.value = f"Ciclo Lectivo: {ciclo['nombre']}"
                text_ciclo_status.color = "white"
                ...
                current_cycle_ref["id"] = None
                text_ciclo_status.value = "⚠️ No hay ciclo activo. Vaya a Admin."
                text_ciclo_status.color = "#FFCDD2" # Rojo claro
        ...
        if state["role"] == "admin":
            def add_curso_dlg(e):
                if not current_cycle_ref["id"]:
                    return show_snack("¡Error! No hay un ciclo activo.", True)
                ...
                def save(e):
                    if tf.value:
                        if run_query("INSERT INTO Cursos (nombre, ciclo_id) VALUES (%s, %s)", (tf.value, current_cycle_ref["id"])):
                            page.close(dlg)
                            load_dashboard_data()
        ...
        return ft.View("/dashboard", [
            # Pasamos el control de texto (que actualizamos dinámicamente) como subtítulo
            create_header("Panel Principal", subtitle_control=text_ciclo_status, trailing_action=ft.Row(actions, spacing=0, tight=True)),
    ...
    def view_ciclos():
        ...
        def load_ciclos():
            ...
            for c in AdminService.get_ciclos():
                ...
                actions = []
                if not is_active:
                    actions.append(ft.ElevatedButton("Activar", bgcolor=WARNING_COLOR, color="white", height=30, 
                                                     on_click=lambda e, cid=c['id']: (AdminService.activar_ciclo(cid), load_ciclos(), show_snack("Ciclo activado"))))
                else:
                    actions.append(ft.Container(content=ft.Text("ACTIVO", color="white", weight="bold", size=12), bgcolor=SUCCESS_COLOR, padding=5, border_radius=5))
                
                # Botón de borrar (NUEVO)
                actions.append(ft.IconButton("delete", icon_color=DANGER_COLOR, on_click=lambda e, cid=c['id']: (AdminService.delete_ciclo(cid), load_ciclos())))
        ...
        def add_ciclo_click(e):
            ...
            if AdminService.add_ciclo(tf_nombre.value):
                ...
                show_snack("Error: Probablemente el nombre ya existe", True)
    ...
    def view_curso_detail():
        ...
        def load_alumnos():
            ...
            for r in rows:
                ...
                # Indicador visual TPP
                subtitle = f"DNI: {r['dni'] or '-'}"
                if r.get('tpp', 0) == 1:
                    subtitle += " | ⚠️ TPP"

                tile = create_card(ft.ListTile(
                    leading=avatar, title=ft.Text(r['nombre'], weight=ft.FontWeight.BOLD), subtitle=ft.Text(subtitle),
        ...
        def load_asistencia_ui(e=None):
            ...
            for a in run_query("SELECT * FROM Alumnos WHERE curso_id=%s ORDER BY nombre", (state["curso_id"],), fetch=True):
                ...
                if a['id'] not in mapa and a.get('tpp', 0) == 1:
                    ...
                    if tpp_dias_str and str(dia_semana) not in tpp_dias_str.split(','):
                        val_defecto = "N"
    ...
    def view_form_student():
        ...
        days_checks = [ft.Checkbox(label=d, value=True, data=str(i)) for i, d in enumerate(["Lunes", "Martes", "Miércoles", "Jueves", "Viernes"])]
        
        days_container = ft.Column([ft.Text("Días de Asistencia:", size=12, weight="bold")] + days_checks, visible=False)
        ...
        if is_edit:
            ...
            if d:
                ...
                if d.get('tpp') == 1:
                    sw_tpp.value = True; days_container.visible = True
                    saved_days = (d.get('tpp_dias') or "").split(',')
                    for chk in days_checks: chk.value = chk.data in saved_days
        ...
        def save(e):
            ...
            if is_edit:
                run_query("""UPDATE Alumnos SET nombre=%s, dni=%s, tutor_nombre=%s, tutor_telefono=%s, observaciones=%s, tpp=%s, tpp_dias=%s WHERE id=%s""", (nm.value, dn.value, tn.value, tt.value, ob.value, tpp_val, tpp_days_str, aid))
            else:
                run_query("""INSERT INTO Alumnos (curso_id, nombre, dni, tutor_nombre, tutor_telefono, observaciones, tpp, tpp_dias) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", (cid, nm.value, dn.value, tn.value, tt.value, ob.value, tpp_val, tpp_days_str))
            page.go("/curso")
        ...
        return ft.View("/form_student", [
            ...
            ft.Container(content=create_card(ft.Column([nm, dn, ft.Divider(), tn, tt, ft.Divider(), ob, ft.Divider(), ft.Container(content=ft.Column([sw_tpp, days_container]), bgcolor=ft.colors.BLUE_50, padding=10, border_radius=10), ft.Container(height=10), ft.ElevatedButton("Guardar", on_click=save, width=float("inf"), bgcolor=PRIMARY_COLOR, color="white")])), padding=20, bgcolor=BG_COLOR, expand=True)
        ])
    ...
    def view_student_detail():
        ...
        if s.get('tpp') == 1:
            tpp_info = ft.Container(content=ft.Row([ft.Icon("warning", color="orange"), ft.Text("Régimen TPP Activo", weight="bold", color="orange")], alignment="center"), bgcolor=ft.colors.ORANGE_50, padding=10, border_radius=10, margin=ft.margin.only(bottom=10))
        ...
        return ft.View("/student_detail", [
            ...
            ft.Container(content=create_card(ft.Column([
                ...
                ft.Divider(), ft.Text("Contacto", weight="bold"), ft.ListTile(leading=ft.Icon("phone"), title=ft.Text(s.get('tutor_nombre') or '-'), subtitle=ft.Text(s.get('tutor_telefono') or '-')),
