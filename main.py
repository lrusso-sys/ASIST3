...
try:
    ...
    psycopg2 = None
    print("❌ ERROR CRÍTICO: psycopg2 no instalado. La app fallará al conectar a DB.", flush=True)
...
class DatabaseManager:
    ...
    def init_db(self):
        print("Intentando inicializar tablas...", flush=True)
        conn = self.get_connection()
        if not conn:
            print("❌ CRÍTICO: No hay conexión a DB. Las tablas no se crearán.", flush=True)
            return False
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if cursor.fetchone()[0] == 0:
                    print("Creando ciclo inicial...", flush=True)
    ...
    def toggle_requisito(self, rid, aid, val):
        if val:
            # Postgres UPSERT simplificado con ON CONFLICT DO NOTHING
...
def view_login(page: ft.Page):
    user_input = ft.TextField(label="Usuario", width=300, bgcolor="white", border_radius=8)
    pass_input = ft.TextField(label="Contraseña", password=True, width=300, bgcolor="white", border_radius=8, can_reveal_password=True)
...
def view_asistencia(page: ft.Page):
    ...
    def save_all(e):
        ...
        for aid, dd in inputs_map.items():
            if db.registrar_asistencia(aid, fecha, dd.value):
                count += 1
...
def view_form_student(page: ft.Page):
    ...
    return ft.View("/form_student", [
        ...
        ft.Container(content=create_card(ft.Column([nm, dni, ft.Divider(), tn, tt, ft.Divider(), obs, ft.ElevatedButton("GUARDAR", on_click=save, bgcolor=THEME["success"], color="white", width=float("inf"))])), padding=20, bgcolor=THEME["bg"], expand=True)
    ])
...
def view_form_curso(page: ft.Page):
    ...
    return ft.View("/form_curso", [
        ...
        ft.Container(content=create_card(ft.Column([tf, ft.ElevatedButton("Crear", on_click=save, bgcolor=THEME["success"], color="white")])), padding=20, bgcolor=THEME["bg"], expand=True)
    ])
...
def view_form_req(page: ft.Page):
    ...
    return ft.View("/form_req", [
        ft.AppBar(leading=ft.IconButton("arrow_back", icon_color="white", on_click=lambda _: page.go("/pedidos")), title=ft.Text("Nuevo Requisito"), bgcolor=THEME["primary"], color="white"),
        ft.Container(content=create_card(ft.Column([tf, ft.ElevatedButton("Guardar", on_click=save)])), padding=20, bgcolor=THEME["bg"], expand=True)
    ])
...
def view_search(page: ft.Page):
    ...
    return ft.View("/search", [
        ft.AppBar(leading=ft.IconButton("arrow_back", icon_color="white", on_click=lambda _: page.go("/dashboard")), title=ft.Text(f"Búsqueda: {term}"), bgcolor=THEME["primary"], color="white"),
...
# Agregamos este método al DatabaseManager antes de main para inicializar correctamente
# (Este es un parche rápido para mantener el estilo de clase)
DatabaseManager.init_db_instance = lambda self: self._init_db()
