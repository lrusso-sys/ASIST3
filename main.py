...
# ==============================================================================
# Paleta de colores usando nombres estándar de Flet (Material Design)
...
def main(page: ft.Page):
    ...
    state = {
        "role": None, "username": None, "curso_id": None, "curso_nombre": None, "alumno_id": None
    }
    ...
    def view_login():
        ...
        return ft.View("/", [
            ft.Container(
                content=ft.Column([
                    ft.Icon("school", size=80, color=PRIMARY_COLOR),
                    ft.Text("Bienvenido", size=30, weight="bold", color=PRIMARY_COLOR),
                    ft.Text("Sistema de Gestión UNSAM", size=16, color="grey600"),
                    ft.Divider(height=40, color="transparent"),
                    user, pwd, ft.Container(height=20),
                    ft.ElevatedButton("INICIAR SESIÓN", on_click=login_click, width=300, height=50, bgcolor=PRIMARY_COLOR, color="white"),
    ...
    def view_dashboard():
        ...
        def load_cursos():
            ...
            for c in cursos:
                ...
                card = ft.Container(
                    content=ft.Row([
                        ft.Row([
                            ft.Container(content=ft.Icon("class", color="white"), bgcolor=PRIMARY_COLOR, border_radius=10, padding=12),
                            ft.Text(c['nombre'], size=18, weight="w600", color=TEXT_COLOR)
                        ]),
        ...
        return ft.View("/dashboard", [
            create_header("Panel Principal", f"Ciclo Lectivo: {ciclo_nombre}", trailing_action=ft.IconButton("logout", icon_color="white", tooltip="Salir", on_click=lambda _: page.go("/"))),
            ft.Container(content=ft.Column([
                ft.Text("Mis Cursos", size=22, weight="bold", color=TEXT_COLOR),
    ...
    def view_curso_detail():
        ...
        # --- TAB 1: ALUMNOS ---
        alumnos_col = ft.Column(scroll="auto")
        def load_alumnos():
            ...
            for r in rows:
                ...
                tile = create_card(ft.ListTile(
                    leading=avatar, title=ft.Text(r['nombre'], weight="bold"), subtitle=ft.Text(f"DNI: {r['dni'] or 'S/D'}"),
        ...
        # --- TAB 2: ASISTENCIA ---
        asist_col = ft.Column(scroll="auto")
        ...
        # --- TAB 3: REPORTES ---
        report_col = ft.Column(scroll="auto")
        def load_report_ui():
        ...
        def open_edit_student(s):
             # Placeholder para editar
             pass

        def open_student_detail(s):
